...
def load_dataset():
    ...
    x_train_batch = torch.load(os.path.join(
        parent_dir, 'imagenet_1k/train/imagenet_x_batch.pt'), map_location=torch.device('cpu'))
    y_train_batch = torch.load(os.path.join(
        parent_dir, 'imagenet_1k/train/imagenet_y_batch.pt'), map_location=torch.device('cpu'))
    x_test_batch = torch.load(os.path.join(
        parent_dir, 'imagenet_1k/test/imagenet_x_batch.pt'), map_location=torch.device('cpu'))
    y_test_batch = torch.load(os.path.join(
        parent_dir, 'imagenet_1k/test/imagenet_y_batch.pt'), map_location=torch.device('cpu'))
