    ...
    def __init__(self, name, model):
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Embedding):
                module.register_forward_hook(self.embedding_hook)
                break

    def embedding_hook(self, module, input, output):
        self.embedding_output = output

    def hook_for_output(self, model, embedding):
...
#                 logging.info("Normalization automatically loaded from `model.mean` and `model.std`.")
    ...
    def generate_adversarial_samples(self,inputs,labels):
        adv_samples = self.__call__(inputs,labels)
        return adv_samples
    ...
    def attack_dataloader(self,dataloader):
        ...
        for inputs, labels in dataloader:
            adv_samples = self.generate_adversarial_samples(inputs,labels)
            with torch.no_grad():
                adv_data.append(adv_samples.cpu())
                adv_preds.append(self.model(adv_samples).cpu())
                original_data.append(inputs.cpu())
                true_labels.append(labels.cpu())
        adv_data = torch.cat(adv_data)
        adv_preds = torch.cat(adv_preds)
        original_data = torch.cat(original_data)
        true_labels = torch.cat(true_labels)
        return adv_data, adv_preds, original_data, true_labels


def run_attack(model, method, dataloader, transfer_models=None, **kwargs):
    r"""
    Run attack.
    Arguments:
        model (nn.Sequential): model to attack.
        method (Attack): attack method.
        dataloader (torch.utils.data.DataLoader): data loader.
        transfer_models (dict): dict of transfer models. like {'model_name':model}
        kwargs: arguments for attack method.


    Examples::
        >>> run_attack(model, BIM, dataloader,transfer_models={"resnet50":resnet50_model}, eps=8/255, alpha=2/255, steps=10)
    """
    method = method(model=model, **kwargs)
    if method == AdvGAN:
        method.train(dataloader)
    adv_data,adv_preds,original_data,true_labels = method.attack_dataloader(dataloader)
    print(
        f"Succeed rate: {(adv_preds.argmax(dim=-1)!=true_labels).float().mean()}")
...
def plot_adversarial_sample(adv_data, original_data):
    fig, axes = plt.subplots(1, 2, figsize=(10, 5))
    axes[0].imshow(adv_data.permute(1, 2, 0).numpy())
    axes[0].set_title("Adversarial Example")
    axes[1].imshow(original_data.permute(1, 2, 0).numpy())
