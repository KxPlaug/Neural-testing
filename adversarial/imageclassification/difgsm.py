...
class DIFGSM(Attack):
    ...
    Examples::
        >>> attack = DIFGSM(model, eps=8/255, alpha=2/255, steps=10, decay=0.0, resize_rate=0.9, diversity_prob=0.5, random_start=False)
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            # Calculate loss
            if self.targeted:
                cost = -self.get_loss(self.input_diversity(adv_images), target_labels)
            else:
                cost = self.get_loss(self.input_diversity(adv_images), labels)
