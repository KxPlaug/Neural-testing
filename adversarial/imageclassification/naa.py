...
class NAA(Attack):
    r"""
        ...
        Examples::
            >>> attack = NAA(model, eps=8/255, alpha=2/255, steps=10)
            >>> adv_images = attack(images, labels)
    """
    def __init__(self, model, eps=8/255,
                 alpha=2/255, steps=10, num_classes=1000):
    ...
    def forward(self, x, y):
        ...
        y = y.to(device)
        labels = torch.nn.functional.one_hot(y, self.num_classes).to(device)
                ...
                if self.ens == 0:
                    ...
                    if weight_np is None:
                        weight_np = torch.autograd.grad(
                            logits*labels, features, grad_outputs=torch.ones_like(logits*labels))[0]
                    else:
                        weight_np += torch.autograd.grad(
                            logits*labels, features, grad_outputs=torch.ones_like(logits*labels))[0]
