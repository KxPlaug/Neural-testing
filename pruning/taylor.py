...
class Taylor:
    ...
    Arguments:
        alpha (float): the hyperparameter of Taylor
        pruning_ratio (float): the pruning ratio

    __call__:
        Arguments:
            net (nn.Sequential): the model to be pruned
            train_dataloader (DataLoader): the dataloader of the training dataset
            save_path (str): path to save the pruned model mask
        Returns:
            pruned_net (nn.Sequential): the pruned model
    ...
    def __call__(self, net, train_dataloader, save_path=None):
        self.net = net
