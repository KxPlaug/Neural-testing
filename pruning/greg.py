...
class GReg:
    r"""
    GReg pruning algorithm.
    Arguments:
        img_size (int): image size (Default: 224)
        num_classes (int): number of classes (Default: 1000)
        num_chennels (int): number of channels (Default: 3)
        momentum (float): momentum (Default: 0.9)
        weight_decay (float): weight decay (Default: 1e-4)
        reg_granularity_recover (float): granularity of regularization for recovering (Default: 1e-4)
        reg_granularity_prune (float): granularity of regularization for pruning (Default: 1e-4)
        reg_granularity_pick (float): granularity of regularization for picking (Default: 1e-5)
        reg_upper_limit_pick (float): upper limit of regularization for picking (Default: 1e-2)
        reg_upper_limit (float): upper limit of regularization (Default: 1.0)
        lr_prune (float): learning rate for pruning (Default: 0.001)
        test_interval (int): test interval (Default: 2000)
        update_reg_interval (int): interval for updating regularization (Default: 5)
        stabilize_reg_interval (int): interval for stabilizing regularization (Default: 40000)
        n_conv_within_block (int): number of conv layers within a block (Default: 3)
        res (bool): whether the model is a resnet (Default: True)
        stage_pr (str): pruning ratio for each stage (Default: "[0,0.675,0.675,0.675,0.675,0.675]")
        skip_layers (str): layers to skip (Default: "")
        pick_pruned (str): how to pick pruned weights (Default: "minimum")
    __call__:
        Arguments:
    ...
    def __call__(self, model, train_loader_prune, test_loader,save_path=None):
